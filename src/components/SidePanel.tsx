  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ color: '#555', fontSize: 7, letterSpacing: '1px' }}>TASK FEED</span>
        <button
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '10px 0' }}>
        ...
        {active.length > 0 && (
          <div>
            <div style={{ padding: '4px 14px 8px', display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <span style={{ color: '#00ff88', fontSize: 6, letterSpacing: '1px' }}>ACTIVE</span>
            </div>
            {active.map(task => {
              ...
              return (
                <div key={task.id} style={{
                  ...
                  <div style={{ color: cfg.color, fontSize: 5, letterSpacing: '0.5px', marginBottom: 4, textShadow: `0 0 6px ${cfg.color}` }}>
                    {cfg.label}
                  </div>
                  <div style={{ color: '#888', fontSize: 5, lineHeight: 1.8, wordBreak: 'break-word' }}>
                    {task.task_name.substring(0, 38)}{task.task_name.length > 38 ? '...' : ''}
                  </div>
                  <div style={{ color: '#333', fontSize: 5, marginTop: 4, display: 'flex', justifyContent: 'space-between' }}>
        ...
        {completed.length > 0 && (
          <div style={{ marginTop: active.length > 0 ? 16 : 0 }}>
            <div style={{ padding: '4px 14px 8px' }}>
              <span style={{ color: '#2a2a2a', fontSize: 6, letterSpacing: '1px' }}>COMPLETED</span>
            </div>
            {completed.map((task, i) => {
              ...
              return (
                <div key={task.id} style={{
                  ...
                  <div style={{ color: '#2a2a2a', fontSize: 5, marginBottom: 3 }}>
                    <span style={{ color: cfg.color + '66' }}>{cfg.label.substring(0, 4)}</span>
                    {' '}{task.task_name.substring(0, 30)}{task.task_name.length > 30 ? '...' : ''}
                  </div>
                  <div style={{ color: failed ? '#ff444466' : '#1e1e1e', fontSize: 5, display: 'flex', justifyContent: 'space-between' }}>
        ...
        {filtered.length === 0 && (
          <div style={{ color: '#222', fontSize: 6, padding: '20px 14px', lineHeight: 2 }}>
